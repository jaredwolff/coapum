...
async fn simulate_client_lifecycle(client_manager: ClientManager) {
    ...
    for i in 1..=3 {
        ...
        client_manager
            .add_client(&device_id, key.as_bytes())
            .await
            .unwrap();
    ...
    for i in 1..=2 {
        ...
        if client_manager
            .update_key(&device_id, new_key.as_bytes())
            .await
            .is_ok()
        {
    ...
    println!("\nDisabling a client...");
    client_manager
        .set_client_enabled("initial_device_001", false)
        .await
        .unwrap();
...
async fn periodic_key_rotation(client_manager: ClientManager) {
    ...
    loop {
        ...
        println!(
            "\n[Key Rotation #{}] Starting periodic key rotation...",
            rotation_count
        );
        ...
        for client in clients.iter().filter(|c| c.starts_with("rotate_")) {
            ...
            if client_manager
                .update_key(client, new_key.as_bytes())
                .await
                .is_ok()
            {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    initial_clients.insert(
        "initial_device_001".to_string(),
        b"initial_key_001".to_vec(),
    );
    initial_clients.insert(
        "initial_device_002".to_string(),
        b"initial_key_002".to_vec(),
    );
    ...
    let (client_manager, server_future) =
        serve_with_client_management("0.0.0.0:5683".to_string(), config, router).await?;
