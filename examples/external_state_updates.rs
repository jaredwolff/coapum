...
impl AppState {
    fn new() -> Self {
        ...
        devices.insert("sensor_01".to_string(), DeviceStatus {
            online: true,
            last_seen: 0,
            battery_level: 100,
        });
        devices.insert("sensor_02".to_string(), DeviceStatus {
            online: true,
            last_seen: 0,
            battery_level: 85,
        });
...
async fn simulate_sensor_updates(state_handle: StateUpdateHandle<AppState>) {
    ...
    loop {
        ...
        state_handle.update(move |state: &mut AppState| {
            ...
            println!("Sensor update: temp={:.1}°C, humidity={:.1}%", 
...
async fn simulate_device_updates(state_handle: StateUpdateHandle<AppState>) {
    ...
    loop {
        ...
        let device_id = if rand::random::<bool>() { "sensor_01" } else { "sensor_02" };
        ...
        state_handle.update(move |state: &mut AppState| {
            if let Some(device) = state.devices.get_mut(device_id) {
                ...
                if device.battery_level < 10 {
                    device.online = false;
                    println!("Device {} went offline (low battery: {}%)", 
                             device_id, device.battery_level);
                } else {
                    println!("Device {} status: battery={}%", 
                             device_id, device.battery_level);
                }
            }
        }).await.unwrap();
...
async fn update_system_stats(state_handle: StateUpdateHandle<AppState>) {
    ...
    loop {
        ...
        state_handle.update(|state: &mut AppState| {
            state.uptime_seconds += 1;
            
            // Print stats every 10 seconds
            if state.uptime_seconds % 10 == 0 {
                println!("System uptime: {}s, requests handled: {}", 
...
async fn external_monitoring(state_handle: StateUpdateHandle<AppState>) {
    ...
    loop {
        ...
        state_handle.update(|state: &mut AppState| {
            ...
            println!("Monitor: {} of {} devices online", online_count, state.devices.len());
            println!("Monitor: Environment - temp={:.1}°C, humidity={:.1}%", 
                     state.temperature, state.humidity);
        }).await.unwrap();
